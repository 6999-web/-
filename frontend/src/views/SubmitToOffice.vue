      ...
      <div class="section">
        ...
        <el-table :data="submittedList" border stripe>
          ...
          <el-table-column prop="status" label="状态" width="200">
            <template #default="{ row }">
              <el-tag :type="getStatusTagType(row.status)">{{ getStatusLabel(row.status) }}</el-tag>
              <div v-if="row.status === 'rejected_by_president' && row.reject_reason" class="reject-reason-text">
...
import { scoringApi, publicationApi } from '@/api/client'
...
// 已提交的：已提交到办公室（ready_for_final、finalized、published、rejected_by_president）
const submittedList = computed(() =>
  allEvaluations.value.filter((e: any) =>
    ['ready_for_final', 'finalized', 'published', 'distributed', 'rejected_by_president'].includes(e.status)
  )
)

// 状态文案（与手动评分页一致）：评分前、已评分、已提交、已驳回
function getStatusLabel (status: string): string {
  if (!status) return '评分前'
  if (status === 'rejected_by_president') return '已驳回'
...
function getStatusTagType (status: string): string {
  ...
  const map: Record<string, string> = { '评分前': 'warning', '已评分': 'success', '已提交': 'primary', '已驳回': 'danger' }
...
async function loadData () {
  ...
  try {
    ...
    const raw = res.data
    let evalList = Array.isArray(raw) ? raw : (raw?.items ?? raw?.value ?? raw?.list ?? [])
    
    // 获取驳回原因
    if (evalList.length > 0) {
      try {
        const ids = evalList.map((e: any) => e.id).join(',')
        const reasonRes = await publicationApi.getRejectionReasons(ids)
        const reasons = reasonRes.data || {}
        // 将驳回原因附加到对应的评估记录
        evalList = evalList.map((e: any) => ({
          ...e,
