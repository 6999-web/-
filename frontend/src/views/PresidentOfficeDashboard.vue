<template>
  <div class="president-office-dashboard">
    ...
    <div v-else>
      ...
      <!-- AI监控与评分分析 (Requirement 5: 已删除按照用户要求) -->
...
const sortedScores = computed(() => {
  ...
  if (!filters.value.sortBy) return scores
  
  return scores.sort((a, b) => {
    const aVal = a[filters.value.sortBy as keyof TeachingOfficeScore]
    const bVal = b[filters.value.sortBy as keyof TeachingOfficeScore]
...
const averageFinalScore = computed(() => {
  const scores = dashboardData.value.teaching_office_scores
    .filter((s: TeachingOfficeScore) => s.final_score !== undefined)
    .map((s: TeachingOfficeScore) => s.final_score || 0)
...
const completedCount = computed(() => {
  return dashboardData.value.teaching_office_scores.filter(
    (s: TeachingOfficeScore) => s.final_score !== undefined
  ).length
})

const finalizableOffices = computed(() => {
  // Show offices that are in score-related statuses
  // Include: ready_for_final (已提交到办公室), finalized, approved, distributed
  return dashboardData.value.teaching_office_scores.filter(
    (office: TeachingOfficeScore) => 
      ['ready_for_final', 'finalized', 'approved', 'distributed', 'rejected_by_president'].includes(office.status)
  )
})
...
const loadHistoricalData = async () => {
  ...
  try {
    ...
    const officeData = response.data.find(
      (d: { teaching_office_id: string; scores: Array<{ year: number; final_score: number }> }) => d.teaching_office_id === selectedOfficeForHistory.value
    )
...
const downloadResultWord = async () => {
  try {
    ...
    // 传递选中的 evaluation_ids
    const idsParam = selectedEvaluationIds.value.length > 0 ? '&evaluation_ids=' + selectedEvaluationIds.value.join(',') : ''
    const url = `${baseURL}/publication/generate-result-word${year ? '?year=' + year : ''}${idsParam}`
...
@media (max-width: 768px) {
  ...
  /* 评教流程进度单列 */
  .evaluation-progress .el-col {
    margin-bottom: 10px;
  }
  
  /* 表格横向滚动 */
  .scores-table-card :deep(.el-table) {
    font-size: 12px;
    overflow-x: auto;
  }
  ...
  /* 评分详情弹窗 */
  .score-detail-modal :deep(.el-form-item) {
    margin-bottom: 12px;
  }
  
  .score-detail-modal :deep(.el-form-item__label) {
    font-size: 12px;
  }
}
