<template>
  <div class="result-view-page">
    ...
    >
      ...
      >
        <template #header>
          <div class="card-header">
            ...
            <el-tag
              v-if="result.status === 'rejected_by_president'"
              type="danger"
              size="large"
            >
              已驳回
        ...
        <!-- 驳回原因 -->
        <div v-if="result.status === 'rejected_by_president' && result.reject_reason" class="rejection-alert">
          <el-alert
            type="error"
            :closable="false"
            show-icon
          >
            <template #title>
              <div class="rejection-title">
                <strong>驳回原因：</strong>{{ result.reject_reason }}
              </div>
            </template>
          </el-alert>
        </div>
        ...
        <div class="final-score-display">
          <div class="score-value">
            {{ result.final_score?.final_score || getManualScoreAverage() }}
          </div>
        ...
        <div class="score-meta">
          ...
          <p v-if="result.status !== 'rejected_by_president'"><strong>公示时间：</strong>{{ formatDate(result.published_at) }}</p>
        </div>
        
        <!-- 人工评分均分显示 -->
        <div v-if="result.manual_scores && result.manual_scores.length > 0" class="manual-score-summary">
          <el-divider content-position="left">人工评分</el-divider>
          <div class="manual-score-info">
            <p><strong>评审人数：</strong>{{ result.manual_scores.length }} 人</p>
            <p><strong>平均得分：</strong>{{ getManualScoreAverage().toFixed(2) }} 分</p>
          </div>
          <el-table :data="result.manual_scores" size="small" stripe border style="margin-top: 12px">
            <el-table-column prop="reviewer_name" label="评审人" width="120" />
            <el-table-column label="评审角色" width="120">
              <template #default="{ row }">
                <el-tag size="small" :type="row.reviewer_role === 'evaluation_team' ? 'success' : 'warning'">
                  {{ row.reviewer_role === 'evaluation_team' ? '评教小组' : '评教办公室' }}
                </el-tag>
              </template>
            </el-table-column>
            <el-table-column label="总分" width="100">
              <template #default="{ row }">
                {{ getManualScoreTotal(row).toFixed(2) }} 分
              </template>
            </el-table-column>
            <el-table-column prop="submitted_at" label="评分时间">
              <template #default="{ row }">
      ...
      <!-- Insight Summary (Requirement 9: 已删除) -->
    </div>
...
// 计算单个人工评分总分
const getManualScoreTotal = (score: any): number => {
  if (!score.scores || !Array.isArray(score.scores)) {
    return 0
  }
  return score.scores.reduce((sum: number, item: any) => sum + (item.score || 0), 0)
}

// 计算人工评分平均分
const getManualScoreAverage = (): number => {
  if (!result.value?.manual_scores || result.value.manual_scores.length === 0) {
    return 0
  }
  const totals = result.value.manual_scores.map((s: any) => getManualScoreTotal(s))
