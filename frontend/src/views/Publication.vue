    ...
    <div class="page-content">
      ...
      <el-card class="main-card" v-loading="loading">
        ...
        >
          ...
          <el-table-column type="expand">
            <template #default="{ row }">
              <div class="expand-content">
                <!-- 驳回原因（如果被驳回） -->
                <div v-if="row.status === 'rejected_by_president' && row.reject_reason" class="expand-section">
                  <h4 class="expand-title rejection-title">⚠️ 驳回原因</h4>
                  <div class="rejection-info">
                    ...
                    >
                      <template #title>
                        <span class="rejection-reason-text">{{ row.reject_reason }}</span>
                      </template>
                    </el-alert>
                  </div>
                </div>
          ...
          <el-table-column label="公示状态" width="200" align="center">
            <template #default="{ row }">
              <el-tag v-if="row.status === 'rejected_by_president'" type="danger">已驳回</el-tag>
              <el-tag v-else-if="row.is_distributed" type="success">已分发到教研室</el-tag>
              ...
              <div v-if="row.status === 'rejected_by_president' && row.reject_reason" class="reject-reason">
                <el-tooltip :content="row.reject_reason" placement="top">
                  <span>驳回原因：{{ row.reject_reason }}</span>
                </el-tooltip>
              </div>
            </template>
          </el-table-column>
...
const loadEvaluations = async () => {
  ...
  try {
    ...
    let evalList = Array.isArray(res.data) ? res.data : []
    
    // 获取驳回原因
    if (evalList.length > 0) {
      try {
        const ids = evalList.map(e => e.id).join(',')
        const reasonRes = await publicationApi.getRejectionReasons(ids)
        const reasons = reasonRes.data || {}
        // 将驳回原因附加到对应的评估记录
        evalList = evalList.map(e => ({
          ...e,
          reject_reason: reasons[e.id]?.reject_reason || null
        }))
      } catch (reasonErr) {
        console.error('Failed to load rejection reasons:', reasonErr)
      }
    }
    
    evaluations.value = evalList
  } catch (e: any) {
...
const isSelectable = (row: any) => {
  ...
  // 注意：已驳回的记录不能选择
  if (row.status === 'rejected_by_president') {
    return false
  }
