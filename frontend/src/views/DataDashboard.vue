...
const fetchRealData = async () => {
  try {
    ...
    // 计算每个教研室的平均分（使用manual_scores）
    const getAverageScore = (s: any) => {
      if (s.final_score != null) return s.final_score
      if (s.manual_scores && s.manual_scores.length > 0) {
        const total = s.manual_scores.reduce((sum: number, m: any) => sum + m.total_score, 0)
        return total / s.manual_scores.length
      }
      return null
    }
    
    // 获取所有有效分数
    const validScores = scores.map((s: any) => ({ ...s, avgScore: getAverageScore(s) })).filter((s: any) => s.avgScore != null)
    const avgScoreList = validScores.map((s: any) => s.avgScore)
    ...
    keyMetrics.value[3].value = avgScoreList.length > 0 ? (avgScoreList.reduce((a:any, b:any) => a + b, 0) / avgScoreList.length).toFixed(1) : '0.0'

    // 排名 - 使用真实平均分
    rankingData.value = validScores.sort((a:any, b:any) => b.avgScore - a.avgScore).slice(0, 10).map((s:any) => ({ 
      name: s.teaching_office_name, 
      score: s.avgScore.toFixed(1), 
      statusLabel: s.status === 'finalized' ? '已定分' : (s.status === 'approved' ? '已审定' : '已分发')
    }))
    ...
    ].map(L => ({ name: L.name, value: avgScoreList.filter((v: number) => v >= L.min && v <= L.max).length, itemStyle: { color: L.color } }))

    // 各学院平均分 - 从真实数据计算
    const collegeData = validScores.reduce((acc: any[], s: any) => {
      const existing = acc.find(item => item.name === s.teaching_office_name)
      if (existing) {
        existing.scores.push(s.avgScore)
      } else {
        acc.push({ name: s.teaching_office_name, scores: [s.avgScore] })
      }
      return acc
    }, []).map((item: any) => ({
      name: item.name,
      score: item.scores.length > 0 ? (item.scores.reduce((a: number, b: number) => a + b, 0) / item.scores.length).toFixed(1) : 0
    }))
