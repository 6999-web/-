...
def get_teaching_office_result(
    evaluation_id: UUID,
    db: Session = Depends(get_db),
    current_user: User = Depends(RoleChecker(["teaching_office", "evaluation_team", "evaluation_office"])),
):
    """
    教研室端查看已公示/已分发/已驳回的考评结果.
    - 教研室用户：只能查看本教研室的已公示/已分发记录
    - 评教小组/评教办公室：可以查看任何教研室的已公示/已分发/已驳回记录
    """
    evaluation = (
    ...
    # 如果是教研室用户，检查是否是本教研室的记录
    if current_user.role == "teaching_office":
        ...
        # 评教小组/评教办公室可以查看已公示、已分发、已驳回的记录
        if evaluation.status not in ("published", "distributed", "rejected_by_president"):
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="该考评尚未公示或分发，暂无法查看结果",
            )
    ...
    # 获取驳回原因
    reject_reason = None
    if evaluation.status == "rejected_by_president":
        from app.models.approval import Approval
        approvals = db.query(Approval).filter(
            Approval.evaluation_ids.contains([str(evaluation_id)]),
...
def get_published_results(
    ...
    query = (
        ...
        .filter(
            ...
            SelfEvaluation.status.in_(["published", "distributed", "rejected_by_president"])
        )
    ...
    # 获取驳回原因
    from app.models.approval import Approval
    eval_ids = [str(ev.id) for ev in evaluations]
    rejection_reasons = {}
    if eval_ids:
        approvals = db.query(Approval).filter(Approval.decision == "reject").all()
        for approval in approvals:
            for eval_id in approval.evaluation_ids:
                if eval_id in eval_ids and approval.reject_reason:
                    rejection_reasons[eval_id] = approval.reject_reason
    ...
    for ev in evaluations:
        ...
        results.append({
            ...
            "status": ev.status,
            "reject_reason": rejection_reasons.get(str(ev.id), None),
