...
def get_evaluations_for_publication(
    ...
    获取可以发起公示的评估列表。
    包含考评小组的评分信息和附件。
    状态包括: manually_scored, ready_for_final, finalized, approved, published, distributed, rejected_by_president
    """
    query = (
        ...
        .filter(
            SelfEvaluation.status.in_([
                "manually_scored", "ready_for_final", "finalized", "approved", "published", "distributed", "rejected_by_president"
            ])
        )
    )
    if year:
...
def generate_result_word(
    year: Optional[int] = None,
    evaluation_ids: Optional[str] = Query(None, description="逗号分隔的evaluation_ids列表，为空则返回全部"),
    db: Session = Depends(get_db),
    current_user=Depends(get_current_user)
):
    """
    生成考评结果Word文档，供校长端「同意发文」时下载。
    如果指定了evaluation_ids，则只生成选中记录的文档；否则生成全部已审定的记录。
    """
    # 解析 evaluation_ids
    target_ids = None
    if evaluation_ids:
        target_ids = [eid.strip() for eid in evaluation_ids.split(',') if eid.strip()]
    ...
    if year:
        query = query.filter(SelfEvaluation.evaluation_year == year)
    if target_ids:
        query = query.filter(SelfEvaluation.id.in_(target_ids))
    evaluations = query.order_by(SelfEvaluation.submitted_at.desc()).all()
...
def get_rejection_reasons(
    evaluation_ids: str = Query(..., description="逗号分隔的evaluation_ids"),
    db: Session = Depends(get_db),
    current_user: User = Depends(require_evaluation_office),
):
    """
    获取指定考评记录的驳回原因。
    """
    ids = [eid.strip() for eid in evaluation_ids.split(',') if eid.strip()]
    if not ids:
        return {}
    ...
    for approval in approvals:
        for eval_id in approval.evaluation_ids:
            if eval_id in ids and approval.reject_reason:
                result[eval_id] = {
                    "reject_reason": approval.reject_reason,
                    "rejected_at": approval.approved_at.isoformat() if approval.approved_at else None,
