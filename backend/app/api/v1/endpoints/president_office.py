...
def get_dashboard_data(
    ...
    query = (
        ...
        .filter(SelfEvaluation.status.in_(["ai_scored", "manually_scored", "ready_for_final", "finalized", "approved", "published", "distributed", "rejected_by_president", "approved"]))
    )
    if year:
...
@router.get("/historical-scores")
def get_historical_scores(
    teaching_office_id: Optional[str] = Query(None, description="教研室ID"),
    db: Session = Depends(get_db)
):
    """
    获取教研室历史评分数据。
    ...
    if teaching_office_id:
        query = query.filter(SelfEvaluation.teaching_office_id == teaching_office_id)
    ...
    for ev in evaluations:
        ...
        existing = next((item for item in result if item["teaching_office_id"] == str(office.id)), None)
        if existing:
            existing["scores"].append({
